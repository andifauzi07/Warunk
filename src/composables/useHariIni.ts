...
import type { DetailStokLengkap, MasterLauk, RekonsiliasiHarian } from '@/types/database';

export const hariIniKey = (tanggal: string) => ['hari-ini', tanggal] as const;

export function useHariIni(tanggal: Ref<string>, laukAktif: Ref<MasterLauk[]>) {
  ...
  const detail = computed<DetailStokLengkap[]>(() => q.data.value?.detail ?? []);
