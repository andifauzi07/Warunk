...
export async function fetchRingkasanHarian(
  tanggal: string,
): Promise<RingkasanHarianRow | null> {
...
export async function fetchRekonsiliasiRange(
  dari: string,
  sampai: string,
): Promise<TrendRow[]> {
  const { data, error } = await supabase
...
export async function fetchRankingLauk(
  dari: string,
  sampai: string,
): Promise<RankingRow[]> {
  const { data, error } = await supabase
  ...
  const map = new Map<
    string,
    { nama: string; terjual: number; rusak: number }
  >()
